#include "MovesGridW.h"
#include "ui_MovesGridW.h"

MovesGridW::MovesGridW(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MovesGridW)
{
    ui->setupUi(this);
...
MovesGridW::~MovesGridW()
{
    delete ui;
}

CommandTableModel *MovesGridW::getModel()
{
    return mModel;
}

void MovesGridW::ShowConfigData()
{
    ;
}

void MovesGridW::AddFile(QString fileName,int RowsCount, int Duration, bool mode)
{
    if(mode)
    {
...
void MovesGridW::OnRowChanged()
{

}

void MovesGridW::on_FilesTable_doubleClicked(const QModelIndex &index)
{
...
void MovesGridW::ShowContextMenu(const QPoint &pos)
{
    if( mModel->rowCount() == 0 ) return;
...
void MovesGridW::onDialogAccepted()
{
