#include "MotorsGridW.h"
#include "ui_MotorsGridW.h"

MotorsGridW::MotorsGridW(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MotorsGridW)
{
    ui->setupUi(this);
...
MotorsGridW::~MotorsGridW()
{
    delete ui;
}

void MotorsGridW::ShowConfigData()
{
...
void MotorsGridW::OnRowChanged()
{
    int cRow = mSelectionModel->currentIndex().row();
    emit RowChanged(cRow);
}

MotorTableModel *MotorsGridW::getModel()
{
    return mModel;
}

void MotorsGridW::UpdateAngle()
{
    //ui->MotorTableView->resizeColumnsToContents();
    //ui->MotorTableView->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
...
void MotorsGridW::Activate()
{
    ui->MotorTableView->setEnabled(true);
}

void MotorsGridW::Disactivate()
{
    ui->MotorTableView->setEnabled(false);
}

void MotorsGridW::ShowContextMenu(const QPoint &pos)
{
...
void MotorsGridW::onSetEnableAction()
{
    int cRow = mSelectionModel->currentIndex().row();
    int Number = mModel->mDataList.at(cRow)->getNumber();
    bool isEnable = SettingsStorage::Inst()->GetMotors()->at(Number).getEnable();
...
void MotorsGridW::onSetLimitsAction()
{
    int cRow = mSelectionModel->currentIndex().row();
    int Number = mModel->mDataList.at(cRow)->getNumber();
...
void MotorsGridW::onSetLimitsAccepted()
{
    int cRow = mSelectionModel->currentIndex().row();
    int Number = mModel->mDataList.at(cRow)->getNumber();
